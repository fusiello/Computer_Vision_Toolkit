...
x_out = lsq_nonlin(@(x)fobj(x,m,opts.vis,K_fix,opts.kappa,M_fix,index),[g;M0(:)],opts.lsq{:});
...
while i <=  length(options)
    switch options{i}
        ...
        case 'Verbose'
            opts.verbose = true;
            opts.lsq = [opts.lsq, {'Verbose'}];
        case 'GaussNewton'
            gn =  true;
            if opts.verbose
                fprintf('\tBA: Using GaussNewton\n')
            end
        otherwise
...
% Default is 'Reduced', unless 'GaussNewton' is specified
if gn
    opts.lsq = [opts.lsq, {'GaussNewton'}];
else
    opts.lsq = [opts.lsq, {'Reduced'}];
end
